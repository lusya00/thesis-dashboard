...
interface HomestayImageUploadProps {
  homestayId: number
  existingImages: HomestayImage[]
  onImagesChange: (images: HomestayImage[]) => void
  maxImages?: number
  disabled?: boolean
}

export function HomestayImageUpload({
  homestayId,
  existingImages,
  onImagesChange
}: HomestayImageUploadProps) {
  const [images, setImages] = useState<HomestayImage[]>(existingImages)
  ...
  const uploadFile = async (file: File, isPrimary: boolean = false): Promise<HomestayImage> => {
    ...
    const response = await fetch('/api/homestay/images', {
      method: 'POST',
      body: formData
    })
  ...
  }, [images, homestayId, onImagesChange])
